...
type CanteenType = "Day" | "Night";

/** Independent commercial outlets — no hostel mess, single unified on-demand menu. */
type CommercialVenue = "CBRI_Canteen" | "Green_Gala_Cafe" | "CCD" | "Amul_Parlour_MAC";

/** All locations that operate a hostel mess (commercial venues are not messes). */
type MessBhawan = Exclude<BhawanName, CommercialVenue>;

/** Locations with an internal bhawan Day/Night canteen (EWS and commercial venues handled separately). */
type CanteenBhawan = Exclude<BhawanName, "EWS" | CommercialVenue>;
...
type MenuData = Record<MessBhawan, Record<Day, DayMenu>>;
type CanteenData = Record<CanteenBhawan, Record<CanteenType, CanteenMenuItem[]>>;
...
const CBRI_CANTEEN_MENU: CanteenMenuItem[] = [
  { item: "Tea",                   price: 10 },
...
const GREEN_GALA_MENU: CanteenMenuItem[] = [
...
const CCD_MENU: CanteenMenuItem[] = [
...
const AMUL_PARLOUR_MENU: CanteenMenuItem[] = [
...
const COMMERCIAL_MENUS: Record<CommercialVenue, CanteenMenuItem[]> = {
...
const COMMERCIAL_TITLES: Record<CommercialVenue, string> = {
...
const COMMERCIAL_VENUES = new Set<BhawanName>([
  "CBRI_Canteen", "Green_Gala_Cafe", "CCD", "Amul_Parlour_MAC",
]);

function isCommercial(name: BhawanName): name is CommercialVenue {
  return COMMERCIAL_VENUES.has(name);
}

// ── Bhawan Canteen Data ───────────────────────────────────────────────────────

const CANTEEN_DATA: CanteenData = {
...
const BHAWAN_NAMES = [
  ...
  "Arawali", "Sarojini", "Kasturba", "EWS",
  "CBRI_Canteen", "Green_Gala_Cafe", "CCD", "Amul_Parlour_MAC",
] as const satisfies readonly BhawanName[];

const DAYS = [
  "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday",
] as const satisfies readonly Day[];

function formatMessMenu(bhawanName: MessBhawan, day: Day): string {
...
function formatCanteenTable(title: string, items: CanteenMenuItem[]): string {
  const rows = items
...
server.registerTool(
  ...
  {
    description:
      "Fetches the official hostel mess menu (Breakfast, Lunch, Snacks, Dinner) for a specified IIT Roorkee campus location.",
    inputSchema: {
      bhawanName: z.enum(BHAWAN_NAMES),
      day: z.enum(DAYS),
    },
  },
  async ({ bhawanName, day }) => {
    ...
    return {
      content: [{ type: "text" as const, text: formatMessMenu(bhawanName, day) }],
    };
  }
);
...
server.registerTool(
  "get_bhawan_canteen_menu",
  {
    description:
      "Fetches the on-demand Day or Night canteen menu for a specified IIT Roorkee campus location.",
    inputSchema: {
      bhawanName: z.enum(BHAWAN_NAMES),
      canteenType: z.enum(["Day", "Night"] as const satisfies readonly CanteenType[]),
    },
  },
  async ({ bhawanName, canteenType }) => {
    ...
    if (isCommercial(bhawanName)) {
      return {
        content: [
          {
            type: "text" as const,
            text: formatCanteenTable(
              COMMERCIAL_TITLES[bhawanName],
              COMMERCIAL_MENUS[bhawanName]
            ),
          },
        ],
      };
    ...
    const items: CanteenMenuItem[] = CANTEEN_DATA[bhawanName][canteenType];
    const title = `${bhawanName} Bhawan — ${canteenType} Canteen Menu`;

    return {
      content: [{ type: "text" as const, text: formatCanteenTable(title, items) }],
    };
  }
);
