...
// ── Format helper ─────────────────────────────────────────────────────────────
function formatTimetable(department, academicYear, batchSection, day, slots) {
    const header = `## ${DEPT_FULL_NAME[department]} — ${academicYear} Year | Batch: ${batchSection} | ${day}`;
    if (slots.length === 0) {
        return `${header}\n\n_No classes scheduled for this day._`;
    }
    const rows = slots
        .map(({ timeSlot, courseCode, courseTitle, venue }) => `| ${timeSlot} | ${courseCode} | ${courseTitle} | ${venue} |`)
        .join("\n");
    return [
        header,
        "",
...
server.registerTool("get_academic_timetable", {
    description: "Fetches the daily lecture schedule, room allocations, and course timing details for a specified student department, year, and batch section.",
    inputSchema: {
        department: zod_1.z.enum(DEPARTMENTS),
        academicYear: zod_1.z.enum(ACADEMIC_YEARS),
        batchSection: zod_1.z.string().describe("The explicit departmental sub-batch identifier like 'PI1', 'ME1', 'CE2', 'CHE-6', 'CS1'"),
